...
class ViewErrorBoundary extends React.Component<{ children: React.ReactNode; viewName: string }, ErrorBoundaryState> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="flex h-full w-full flex-col items-center justify-center bg-[#090a0f] text-slate-400 select-none p-6 text-center">
...
export const Layout: React.FC = () => {
  ...
  const handleDropTab = (e: React.DragEvent, targetPath: string) => {
    ...
    const fromIdx = openFiles.findIndex((f) => f.path === draggedPath);
    const toIdx = openFiles.findIndex((f) => f.path === targetPath);
  ...
  return (
    <div className="flex h-screen w-screen flex-col bg-[#090a0f] rounded-2xl border border-white/10 overflow-hidden select-none root-layout-bg shadow-2xl relative">
      {/* Titlebar */}
      <Titlebar 
      ...
      <div className="flex flex-1 w-full overflow-hidden">
        ...
        <div className="flex-1 h-full overflow-hidden flex flex-col">
          ...
          {(activeView === "editor" || activeView === "canvas") && openFiles.length > 0 && (
            <div className="flex items-end bg-[#0b0c10]/80 backdrop-blur-sm border-b border-white/5 px-2 h-9 select-none shrink-0 gap-[1px] w-full" style={{boxShadow: 'inset 0 -1px 0 rgba(255,255,255,0.04)'}}>
              {/* Tabs List */}
              <div className="flex items-end flex-1 min-w-0 h-full overflow-hidden gap-[1px]">
                {openFiles.map((tab) => {
                  ...
                  return (
                    <div
                      ...
                      className={`group flex items-center gap-1.5 h-[28px] px-2.5 text-[11px] rounded-t-md border-x border-t transition-all duration-150 cursor-pointer flex-1 min-w-[50px] max-w-[160px] ${
                        isActive
                          ? "bg-[#11131c] text-[#d946ef] border-[#1f2335] border-t-[#d946ef] font-semibold"
                          : "bg-transparent text-slate-500 hover:text-slate-300 hover:bg-[#161825]/50 border-transparent"
                      }`}
                    >
                      ...
                      <button
                        ...
                        className={`ml-1 p-0.5 rounded-full hover:bg-slate-800 text-slate-500 hover:text-slate-300 transition-opacity ${
            ...
            <ViewErrorBoundary key={activeView} viewName={activeView}>
              {activeView === "editor" && (
                openFiles.length > 0 ? (
                  ...
                  <div className="flex h-full w-full flex-col items-center justify-center bg-[#090a0f] text-slate-500 select-none">
                  <div className="flex flex-col items-center gap-4 max-w-sm text-center">
                    <div className="p-4 rounded-full bg-white/5 border border-white/[0.06] backdrop-blur-md shadow-xl">
      ...
      <div className="absolute bottom-0 right-0 z-[90] flex items-center select-none">
        <div className="cmyk-glow-button-wrapper">
          <button
            ...
            className={`relative z-10 flex items-center gap-1.5 px-3.5 py-1.5 rounded-tl-xl border-t border-l shadow-2xl backdrop-blur-md cursor-pointer transition-all duration-200 hover:scale-105 active:scale-95 group ${
              isChatOpen
                ? "bg-[#141624]/95 text-white border-cyan-500/50 shadow-cyan-500/20"
                : "bg-[#0c0e17]/95 hover:bg-[#141726]/95 border-white/10 text-slate-200 hover:text-white"
            }`}
      ...
      {isChatOpen && (
        <div 
          className={`fixed transition-all duration-300 backdrop-blur-md flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300 ${
            isChatDetached
              ? "top-14 right-8 z-[250] w-[500px] sm:w-[560px] h-[680px] max-h-[calc(100vh-80px)] bg-[#090a0f]/95 border border-indigo-500/50 rounded-2xl shadow-[0_35px_80px_rgba(0,0,0,0.9)] shadow-indigo-500/20 resize drop-shadow-2xl"
              : "bottom-10 right-2 z-[100] w-[390px] sm:w-[430px] h-[550px] max-h-[calc(100vh-60px)] bg-[#08090e]/80 border border-[#1f2335]/80 rounded-2xl shadow-[0_25px_60px_rgba(0,0,0,0.75)]"
          }`}
        >
          <CopilotChat 
